...
- **추진 배경**:
    - **스파이크성 수요 집중**: 대형 스포츠 경기, 콘서트, 페스티벌 등은 특정 시간대에 주차 수요가 평소의 수배로 폭증하며 공식 주차장의 조기 만차 및 주변 사설 주차장의 바가지 요금(평소 대비 3~5배) 문제가 만성적으로 발생합니다.
...
- **타겟별 제공 가치**:
    - **공간 제공자 (Host)**: 복잡한 약정 없이 비워두는 몇 시간 동안 유휴 공간을 즉시 수익화.
    - **이용자 (Guest)**: 바가지 요금 없는 사전 확정가로 행사장 도보권 주차 공간을 100% 선점.
    - **주최측 및 지자체**: 행사장 인근의 고질적인 불법 주정차 분산 및 교통 정체 완화.
- **파일럿 검증 핵심 가설**:
...
## 2. Pain Point → Solution (Phase 0 초경량화 규약)
...
| **공통** | 공간 훼손 및 입출차 신뢰 문제 | 차량번호 대조 + 원터치 체크 + 수동/단축 폴링 동기화 | • **대조**: 예약 즉시 호스트 화면에 게스트 차량번호 노출• **상태 반영**: 게스트 버튼 조작 시 호스트 화면에서 **수동 새로고침 및 30초 주기 Polling**으로 최신 상태 갱신• **훼손 방어**: 사진 서버 구축 없이 카카오톡 현장 사진 전송 규칙 적용 |
...
- **단순 변심 취소/환불 불가 (No Refund)**:
    - 본 플랫폼은 행사 한정 단기 패키지 상품이므로, **예약 및 결제 완료 후 이용자의 단순 변심에 의한 취소 및 환불은 전면 불가**합니다.
    - 이는 게스트의 노쇼 및 잦은 취소로부터 호스트의 공간 독점권을 보호하고, Phase 0에서 복잡한 시간대별 취소 수수료 계산 엔진을 배제하기 위함입니다.
- **현장 결함 시 100% 전액 환불 (수동 처리)**:
    - 타 차량 무단 점유, 진입로 폐쇄 등 **호스트/현장 귀책으로 주차가 불가능한 경우에 한해 관리자가 유선 확인 후 100% 전액 환불**합니다.
    - 환불은 시스템 PG 연동이 아닌 **관리자가 게스트 계좌로 직접 송금**하며, 시스템에는 `is_refunded = true` 플래그만 기록합니다.
...
## 6. 상태 머신 및 전이 규칙 (State Machines)

### (1) Event 상태 전이

```
flowchart LR
    UPCOMING["<b>[UPCOMING]</b><br/>• 공간등록 O<br/>• 신규예약 O"]
    ONGOING["<b>[ONGOING]</b><br/>• 공간등록 X<br/>• 신규예약 X<br/>• 입출차조작 O"]
    CLOSED["<b>[CLOSED]</b><br/>• 공간등록 X<br/>• 신규예약 X<br/>• 잔여예약 일괄 COMPLETED"]

    UPCOMING -->|POST /block-reservations| ONGOING
    ONGOING -->|POST /close| CLOSED
```
...
```
flowchart TD
    INIT([생성]) --> CONFIRMED[CONFIRMED]

    %% 정상 흐름
    CONFIRMED -->|check-in| CHECKED_IN[입차완료]
    CHECKED_IN -->|check-out| COMPLETED[COMPLETED]

    %% 일괄 종결 흐름
    CONFIRMED -->|"POST /events/{id}/close 일괄종결"| COMPLETED

    %% 이슈 접수 및 해결 흐름
    CONFIRMED -->|"POST /report-issue"| ISSUE_REPORTED[ISSUE_REPORTED]
    ISSUE_REPORTED -->|"PATCH /resolve"| RESOLVED["CANCELLED 또는 COMPLETED"]

    %% 정산 완료는 별도 status가 아니라 COMPLETED 상태에서의 플래그 갱신
    COMPLETED -->|"PATCH /confirm-payout<br/>(status==COMPLETED AND is_refunded==false 일 때만 허용)<br/>is_payout_done: false → true"| COMPLETED
```

**전이 조건 및 가드 (State Guards)**:
...
3. `report-issue`: `status == 'CONFIRMED'` 일 때만 호출 가능 (호출 즉시 `status = 'ISSUE_REPORTED'` 전환 및 `issue_detail`을 `issue_note`에 누적).
...
- **[화면 1] 공간 등록 화면**: 행사명 헤더, 상세 주소 입력창, 현장 사진 URL 입력(파일 업로드 아님 — 카카오톡 등으로 전달받은 이미지를 별도 호스팅에 올린 뒤 URL만 입력), 예상 도보 시간(분 단위 정수 입력), 진입 유의사항, 패키지 대여 요금, 등록 신청 버튼.
- **[화면 2] 내 공간 및 예약 모니터링 화면**:
    - 등록 공간 요약 (주소, 심사 상태).
    - **활성 예약 카드**: 게스트 차량번호 강조 노출, 입차 상태(O/X), 출차 상태(O/X).
    - **수동 새로고침 버튼** 및 *"30초 주기로 상태가 자동 갱신됩니다"* 캡션.
    - 미출차 등 이상 발생 시 **[관리자 직통 유선 연결]** 버튼.

### (B) Guest 흐름

```
flowchart TD
    subgraph STEP1["1. 탐색 및 예약"]
        A([가입/차량등록]) --> B[행사 목록]
        B --> C[공간 탐색]
        C --> D["상세 확인 및 규정 동의"]
        D --> E[예약 즉시 확정]
    ...
    subgraph STEP2["2. 현장 이용 (정상 완료)"]
        E -->|정상 도착| F[입차 완료 체크]
        F --> G[출차 완료 체크]
        G --> H([이용 종료])
    end

    subgraph STEP3["3. 현장 결함 대응 (예외 환불)"]
        E -->|현장 진입 불가| I[이슈 접수]
        I --> J[관리자 통화]
        J --> K([100% 수동 환불])
    end
...
- **[화면 3] 공간 목록 화면**: 행사명 및 일시 헤더, 거리순 정렬 토글(`sort=distance`), 공간 카드(사진, 도보 N분 뱃지, 고정 요금, 주소 요약).
- **[화면 4] 공간 상세 및 결제 화면**:
    - 사진(단일, `photo_url` 1장), 도보 소요 시간 강조, 상세 주소, 진입 유의사항, 요금 안내, 예약 차량번호 확인창.
    - **[필수 UI 요소] 취소/환불 규정 동의 박스**:
        
        > *"본 상품은 행사 한정 단기 대여 공간으로 **예약 확정 후 단순 변심에 의한 취소 및 환불이 전면 불가**합니다. 단, 현장 무단 점유나 진입 불가 등 현장 결함 시 관리자 유선 확인을 통해 100% 전액 환불됩니다."*
        > 
        - `[필수] 취소/환불 불가 규정을 확인하였으며 이에 동의합니다` 체크박스 미체크 시 결제 버튼 비활성화.
- **[화면 5] 내 예약 상세 화면**: 공간 주소, 호스트 유의사항 안내, 차량번호, [입차 완료] / [출차 완료] 버튼, [현장 문제 신고] 버튼.
- **[화면 6] 현장 이슈 신고 팝업**:
    - 신고 사유(`issue_detail`) 텍스트 입력창.
    - 접수 완료 즉시 비상 유선 연락처 2개(주 담당자: 010-XXXX-XXXX / 부 담당자: 010-YYYY-YYYY) 바로 전화 걸기 버튼 노출. **고정값(config)**: DB에 저장하지 않는 코드 레벨 상수이며, 행사/공간별로 다르지 않음(Phase 0 범위).

### (C) Event Admin 흐름

- **[화면 7] 공간 등록 심사 큐 화면**: 심사 대기 목록 테이블, 주소, 사진, 도보 분, 요금, [승인] / [반려] 버튼.
- **[화면 8] 전체 예약 및 현장 현황 화면**: 행사별 전체 예약 테이블(예약자 차량번호, **호스트 정보**(이름/전화번호, `event_parking_spaces.host_id` 조인), 공간 주소), 상태 필터(`reservations.status` 값과 1:1로 일치하는 `CONFIRMED` / `ISSUE_REPORTED` / `CANCELLED` / `COMPLETED` 4종만 제공. 단순 변심 취소 파이프라인이 Phase 0에서 제외되므로 `CANCEL_REQUESTED` 상태는 존재하지 않음), 상태 뱃지, 입차 O/X, 출차 O/X, 이슈 발생 행 하이라이트 표시, 행 클릭 시 관리자 수동 조치 팝업([resolve] CANCELLED/COMPLETED 선택, [is_refunded] 체크, 메모 입력, [정산 이체 완료] 체크).

## 8. 데이터 모델 (ERD 및 제약조건)

모든 식별자는 표준 **RFC 4122 UUID v4**를 사용합니다.
...
| `password_hash` | VARCHAR | NOT NULL | - | 로그인 비밀번호 (해시 저장, `api.yaml`의 `/auth/signup`·`/auth/login` `password` 필드 대응) |
...
- **상태 코드**:
    - `200 OK`: 모든 정상 응답.
    - `400 Bad Request`: 비즈니스 로직 위반 (파라미터 누락, 마감된 행사, 상태 가드 위반 등).
...
- `PATCH /api/admin/events/{event_id}`: 행사 메타정보 수정 (Admin - 명칭, 장소, 일시만 수정 가능, `status` 수정 불가)
...
- `GET /api/events/{event_id}/spaces`: 행사 인근 예약 가능 공간 목록 (Guest - `walking_minutes` 오름차순. 행사가 `UPCOMING`이 아닐 시 400 반환)
- `POST /api/events/{event_id}/spaces`: 특정 행사에 내 주차공간 등록 (Host - 행사가 `UPCOMING`이 아닐 시 400 반환)
...
### (3) 주요 API 페이로드 및 응답 규격

#### ① `GET /api/spaces/mine` (호스트 내 공간 및 활성 예약 조회)

호스트가 화면에서 새로고침/폴링 시 차량번호와 입출차 상태를 즉시 확인할 수 있도록 `active_reservation`을 포함합니다.
...
```json
// Response 400 Bad Request (예약 이력이 존재하는 경우)
{
  "success": false,
...
필드명을 스키마 규격인 `issue_detail`로 일원화합니다.
...
#### ④ `GET /api/admin/events/{event_id}/reservations` (관리자 예약 모니터링, 화면 8)

`reservations`에는 호스트 필드가 없으므로 `event_parking_spaces.host_id` → `users`를 조인해 호스트 정보를 응답에 포함합니다. `status`는 `CONFIRMED` / `ISSUE_REPORTED` / `CANCELLED` / `COMPLETED` 4종만 존재합니다.
...
단일 트랜잭션으로 행사를 `CLOSED` 처리하고 미출차 건을 `COMPLETED`로 종결합니다.
...
1. **승인 프로세스 전환**: 파일럿 운영 중 노쇼율이 5%를 초과할 경우 호스트 사전 승인제 또는 예비 버퍼 공간 자동 재배정 도입.
2. **PG 결제 및 자동 환불 도입**: 단일 행사당 유선 이슈 대응 건수가 5건을 초과하거나 수동 계좌 이체 운영 시간이 2시간을 초과할 경우 카드사 PG 연동 및 부분 취소 엔진 구축.
3. **본인인증 모듈 탑재**: 허위 정보 등록이나 장난성 예약이 3건 이상 발생할 경우 통신사 PASS 본인확인 모듈 도입.
4. **지도 SDK 연동**: 등록 공간 수가 30면 이상으로 확대되어 텍스트 정렬만으로 탐색 피로도가 증가할 경우 카카오/네이버 지도 SDK 연동.